...
<body>
	...
	// ÚNICA INSTANCIA Y CONEXIÓN PARA TODO EL SCRIPT
	...
	if ($ls_resultado != 0) {
		while ($row = pg_fetch_row($obj_miconexion->li_idconsult)) {
			$serie1_bar[$row[2]] = floatval($row[0]); 
			$serie2_bar[$row[2]] = floatval($row[1]); 
			$total_ingreso += $row[0];
			$total_egreso += $row[1];
		}
		@pg_free_result($ls_resultado);
	} else {
		fun_error(1, $li_id_conex, $ls_sql, $_SERVER['PHP_SELF'], __LINE__);
	}
	...
	if ($ls_resultado != 0) {
		while ($row = pg_fetch_row($obj_miconexion->li_idconsult)) {
			$serie1_finanza[$row[2]] = floatval($row[0]); 
			$serie2_finanza[$row[2]] = floatval($row[1]); 
			$total_venta += $row[0];
			$total_gasto += $row[1];
		}
		@pg_free_result($ls_resultado);
	...
	/*-------------------------------------------------------------------------------------------
	EXTRACCIÓN DE DATOS: VENTAS X COBRAR
	---------------------------------------------------------------------------------------------*/
	...
	$ls_resultado = $obj_miconexion->fun_consult($ls_sql);
	$data_ventas_vendedor = [];
	if ($ls_resultado != 0) {
		while ($row = pg_fetch_row($ls_resultado)) {
			$data_ventas_vendedor[] = $row;
		}
		// CORRECCIÓN: NO usamos fun_closepg aquí para no comprometer el recurso ni la conexión interna.
		@pg_free_result($ls_resultado);
	} else {
	...
	/*-------------------------------------------------------------------------------------------
	EXTRACCIÓN DE DATOS: DINERO PRESTADO
    ---------------------------------------------------------------------------------------------*/
	...
	$ls_resultado = $obj_miconexion->fun_consult($ls_sql);
	$data_dinero_prestado = [];
	if ($ls_resultado != 0) {
		while ($row = pg_fetch_row($ls_resultado)) {
			$data_dinero_prestado[] = $row;
		}
		@pg_free_result($ls_resultado);
	} else {
		fun_error(1, $li_id_conex, $ls_sql, $_SERVER['PHP_SELF']);
	...
	/*-------------------------------------------------------------------------------------------
	EXTRACCIÓN DE DATOS: CUENTAS POR PAGAR
	--------------------------------------------------------------------------------------------*/
	$ls_sql = "SELECT UPPER(s01_persona.tx_nombre ||' '|| s01_persona.tx_apellido),
		sum(detalle) - sum(abono) as Debe 
		FROM v01_pago
	...
	// CIERRE DEFINITIVO: Ahora sí llamamos al método enviándole la conexión real al final de todo el procesamiento
	...
	if (!(isset($data3))) { $data3[] = 0; }
	if (!(isset($data4))) { $data4[] = 0; }
				...
				<div class="row">
					<div class="col-xs-12">
						...
						<div class="row">
							<div class="col-xs-12 infobox-container">
								<div class="infobox infobox-blue">
									<div class="infobox-icon"><i class="ace-icon fa fa-dollar"></i></div>
								...
								<div class="infobox infobox-orange">
									<div class="infobox-icon"><i class="ace-icon fa fa-bolt "></i></div>
									<div class="infobox-data">
						...
						<div class="row">
							<div class="widget-box transparent">
								<div class="widget-header widget-header-flat">
									<h4 class="widget-title lighter"><i class="ace-icon fa fa-star orange"></i>Detalle de Finanzas</h4>
								...
								<div class="widget-body">
									<div class="widget-main padding">
										...
										<div class="col-sm-4">
											<div class="table-header">Ventas x Cobrar</div>
											<table id="example" class="table table-bordered table-striped">
												...
												<tbody>
													<?php
													fun_dibujar_tabla($data_ventas_vendedor, 'LISTAR_DEUDAS_VENDEDOR'); 
													?>
												</tbody>
												<tfoot>
													<tr>
														<td colspan="2" align="right"><strong>Total </strong></td>
														<td><span class="label label-info "><?php echo number_format($VentaXcobrar, 2, ",", ".") . ' Bs'; ?></span></td>
													</tr>
												</tfoot>
											</table>
										</div>
										...
										<div class="col-sm-4">
											<div class="table-header">Cuentas x Pagar</div>
											<table id="tabla_dinamica_2" class="table table-bordered table-striped">
												...
												<tbody>
													<?php
													 if ($tarea == "M") {
														fun_dibujar_tabla($data_cuentas_pagar, 'LISTAR_CREDITOS'); 
													} 
													?>
												</tbody>
												<tfoot>
													<tr>
														<th>Total</th>
														<th><span class="label label-warning "><?php echo number_format($pasivos, 2, ",", ".") . ' Bs'; ?></span></th>
													</tr>
												</tfoot>
											</table>
										</div>
										...
										<div class="col-sm-4">
											<div class="table-header">Dinero Prestado</div>
											<table id="tabla_dinamica_1" class="table table-bordered table-striped">
												...
												<tbody>
													<?php
													fun_dibujar_tabla($data_dinero_prestado, 'LISTAR_PRESTADO'); 
													?>
												</tbody>
												<tfoot>
													<tr>
														<th>Total</th>
														<th><span class="label label-info "><?php echo number_format($CtxCob, 2, ",", ".") . ' Bs'; ?></span></th>
													</tr>
												</tfoot>
											</table>
										</div>
						...
						<div class="row">
							<div class="widget-box transparent ">
								<div class="widget-header widget-header-flat">
									<h4 class="widget-title lighter"><i class="ace-icon fa fa-star orange"></i>Finanzas</h4>
									<div class="widget-toolbar">
										<a href="#" data-action="collapse"><i class="ace-icon fa fa-chevron-up"></i></a>
									</div>
								</div>
						...
						<div class="row">
							<div class="widget-box transparent ">
								<div class="widget-header widget-header-flat">
									<h4 class="widget-title lighter"><i class="ace-icon fa fa-star orange"></i>Gráfica Egresos vs Ingresos Mensual</h4>
									<div class="widget-toolbar">
										<a href="#" data-action="collapse"><i class="ace-icon fa fa-chevron-up"></i></a>
									</div>
								</div>
						...
						<div class="row">
							<div class="widget-box transparent ">
								<div class="widget-header widget-header-flat">
									<h4 class="widget-title lighter"><i class="ace-icon fa fa-star orange"></i>Gráfica Ventas vs Compras Mensual</h4>
									<div class="widget-toolbar">
										<a href="#" data-action="collapse"><i class="ace-icon fa fa-chevron-up"></i></a>
									</div>
								</div>
...
<script type="text/javascript">
		$(document).ready(function() {
			...
			var table = $('#example').DataTable({
				...
				"info": false,
				"searching": false,
				"columns": [{ "orderable": false, "data": null, "defaultContent": '' }, { "orderable": false }, { "orderable": false }],
				"order": [[1, 'asc']]
			});
			...
			$('#example tbody').on('click', 'td.details-control', function() {
				...
				$.post("ajax_deudas_ventas.php", { id_vendedor: id_vendedor }, function(data) {
			...
			$('#tabla_dinamica_1').DataTable({ "paging": false, "ordering": false, "info": false, "searching": false, "order": [[1, 'asc']] });
			$('#tabla_dinamica_2').DataTable({ "paging": false, "ordering": false, "info": false, "searching": false, "order": [[1, 'asc']] });

			Highcharts.chart('container_1', {
				chart: { type: 'column', options3d: { enabled: true, alpha: 15, beta: 15, viewDistance: 25, depth: 60 } },
				title: { text: 'Ingreso vs Egresos por Mes' },
				xAxis: { categories: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], crosshair: true },
				yAxis: { min: 0, title: { text: 'Monto (Bs)' } },
				tooltip: { headerFormat: '<b>{point.key}</b><br>', pointFormat: '<span style="color:{series.color}">\u25CF</span> {series.name}: {point.y}' },
				plotOptions: { column: { pointPadding: 0.2, borderWidth: 0 } },
				series: [
					{ color: color_bar1, name: 'Ingreso', data: [<?php echo join(',', array_values($serie1_bar)) ?>] },
					{ color: color_bar2, name: 'Egreso', data: [<?php echo join(',', array_values($serie2_bar)) ?>] }
				]
			});

			Highcharts.chart('container_3', {
				chart: { type: 'column', options3d: { enabled: true, alpha: 15, beta: 15, viewDistance: 25, depth: 60 } },
				title: { text: 'Ventas vs Compras por Mes' },
				xAxis: { categories: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], crosshair: true },
				yAxis: { min: 0, title: { text: 'Monto (Bs)' } },
				tooltip: { headerFormat: '<b>{point.key}</b><br>', pointFormat: '<span style="color:{series.color}">\u25CF</span> {series.name}: {point.y}' },
				plotOptions: { column: { pointPadding: 0.2, borderWidth: 0 } },
				series: [
					{ color: color_bar1, name: 'Ventas', data: [<?php echo join(',', array_values($serie1_finanza)) ?>] },
					{ color: color_bar2, name: 'Compras', data: [<?php echo join(',', array_values($serie2_finanza)) ?>] }
				]
			});
		});
	</script>
