...
|-------------------------------------------------------------------------------------------*/
	if ($tarea == "M") {
		$ls_sql = "SELECT nb_articulo, t05_clase.fk_categoria, fk_clase, tx_descripcion, in_venta, in_gasto, nu_precio_venta, nu_precio_compra
				  FROM t13_articulo
		...
		if ($ls_resultado != 0) {
			...
			$x_venta     		= $row[4];
			$x_gasto     		= $row[5];
			$x_precio_venta    	= $row[6];
			$x_precio_compra    = $row[7];
		...
		if ($ls_resultado != 0) {
			if ($obj_miconexion->fun_numregistros() == 0) {
				...
				$ls_sql = "UPDATE t13_articulo SET 
								...
								in_venta       = '$x_venta',
								in_gasto       = '$x_gasto',
								nu_precio_venta      = $x_precio_venta,
								nu_precio_compra     = $x_precio_compra												
...
-------------------------------------------------------------------------------------------*/
	if ($tarea == "I") {
		...
		if ($ls_resultado != 0) {
			if ($obj_miconexion->fun_numregistros() == 0) {
				$ls_sql = "INSERT INTO t13_articulo(nb_articulo, tx_descripcion, fk_clase , in_venta, in_gasto, nu_precio_venta, nu_precio_compra	)
							VALUES ('" . $o_nombre . "', '" . $x_descripcion . "',  $o_clase_articulo, '$x_venta','$x_gasto', $x_precio_venta, $x_precio_compra		
							);";
							...
							<div class="widget-body">
								<div class="widget-main ">
									<form class="form-horizontal" name="formulario">
										...
										<div class="form-group">
											<label class="col-sm-3 control-label no-padding-right">Precio Compra</label>
											<div class="col-sm-7">
												<input class="col-xs-10 col-sm-7" name="x_precio_compra" value="<?php echo $x_precio_compra; ?>" id="x_precio_compra" type="text" placeholder="Precio" onkeypress="return validardec(event)">
											</div>
										</div>

										<div class="form-group">
											<label class="col-sm-3 control-label no-padding-right">Ganancia %</label>
											<div class="col-sm-7">
												<input class="col-xs-10 col-sm-7" name="x_gain" value="<?php echo $x_gain; ?>" id="x_gain" type="text" placeholder="Ganancia" onkeypress="return validardec(event)" onblur="calcularVenta()">
											</div>
										</div>


										<div class="form-group">
											<label class="col-sm-3 control-label no-padding-right">Precio Venta</label>
											<div class="col-sm-7">
												<input class="col-xs-10 col-sm-7" name="x_precio_venta" value="<?php echo $x_precio_venta; ?>" id="x_precio_venta" type="text" placeholder="Precio" onkeypress="return validardec(event)" onblur="calcularGain()">
											</div>
										</div>
...
<script type="text/javascript">
	...
	function calcularVenta() {
		let valorGain = document.getElementById("x_gain").value; 
		let valorCompra = document.getElementById("x_precio_compra").value;

		let precioVenta = valorCompra * (1 + (valorGain / 100));
		document.getElementById("x_precio_venta").value = precioVenta.toFixed(2);

	}

	function calcularGain() {		
		let valorCompra = document.getElementById("x_precio_compra").value; 
		let valorVenta = document.getElementById("x_precio_venta").value;
		let valorGain = 0;
		if(valorCompra > 0){
			valorGain = ((valorVenta - valorCompra) / valorCompra) * 100
			document.getElementById("x_gain").value = valorGain.toFixed(2);
		}
