...
-------------------------------------------------------------------------------------------*/
	...
	if ($tarea == "I"){		
		...
		/*$ls_sql = "SELECT fk_cliente FROM t20_factura	WHERE (tx_tipo) = 'CTAXPAGAR' AND fk_cliente = $o_cliente";
		...
		if($ls_resultado != 0){
				if($obj_miconexion->fun_numregistros() == 0){*/
					...
					if($obj_miconexion->fun_consult($ls_sql) == 0){
						...
						$parametros = "tarea=A";
						echo "<script language='javascript' type='text/javascript'>alert('$msg');location.href='adm_ctaxpagar_add.php?$parametros';</script>";
					}
...
-------------------------------------------------------------------------------------------*/
	if ($tarea == "U"){
		...
		/*$ls_sql = "SELECT fk_cliente FROM t20_factura	WHERE (tx_tipo) = 'CTAXPAGAR' AND fk_cliente = $o_cliente AND pk_factura <> $x_movimiento";
		$ls_resultado =  $obj_miconexion->fun_consult($ls_sql);
		if($ls_resultado != 0){
				if($obj_miconexion->fun_numregistros() == 0){*/
					...
					if($obj_miconexion->fun_consult($ls_sql) == 0){
						...
						echo "<script language='javascript' type='text/javascript'>alert('$msg');location.href='adm_ctaxpagar_view.php?$parametros';</script>";
					}
		$tarea = 'M';
	}
...
-------------------------------------------------------------------------------------------*/
	if ($tarea == "M"){		
		$ls_sql ="SELECT pk_factura, fk_responsable, fk_cliente, to_char(fe_fecha_factura, 'dd/mm/yyyy'), 
					tx_concepto,  nu_total, nu_subtotal, nu_abono, tx_nota
					FROM t20_factura
					WHERE pk_factura = $x_movimiento";
							...
							<div class="widget-box ">
								<div class="widget-body">
									<div class="widget-main">
											...
											<div class="form-group">
												<label class="col-sm-3 control-label no-padding-right" >Acreedor</label>
												<div class="col-sm-7" >	
											...
											<div class="form-group">
												<label  class="col-sm-3 control-label no-padding-right" for="x_concepto" >Concepto</label>
												<div class="col-sm-9" >
													<textarea name="x_concepto" cols="2" id="x_concepto" class="form-control" rows="1" onKeyPress="return validarAlfa(event)"placeholder="Enter ..."><?php echo $x_concepto;?></textarea>
												</div>
											</div>
											...
											<div class="form-group">
												...
												<div class="col-sm-9" >
													<textarea name="x_descripcion" cols="10" id="x_descripcion" class="form-control" rows="4" onKeyPress="return validarAlfa(event)"placeholder="Enter ..."><?php echo $x_descripcion;?></textarea>
												</div>
											</div>		
...
<script type="text/javascript">
	...
	function Eliminar_Venta(identificador){
		alert('Operacion Desactivada. Consulte al Administrador');
