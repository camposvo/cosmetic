...
-------------------------------------------------------------------------------------------*/	
	if ($tarea == "E"){
		$ls_sql = "SELECT * FROM gan_ganado WHERE fk_sector = '$pk_sector' ";
		
		$ls_resultado =  $obj_miconexion->fun_consult($ls_sql);
		if ($ls_resultado != 0){
			if($obj_miconexion->fun_numregistros($ls_resultado) == 0){ 
				$ls_sql = "DELETE FROM gan_sector WHERE pk_sector = '$pk_sector' ";
				$ls_resultado =  $obj_miconexion->fun_consult($ls_sql);
					if($ls_resultado != 0){
...
/*-------------------------------------------------------------------------------------------
	LISTAR SECTORES
-------------------------------------------------------------------------------------------*/			
	$ls_sql = "SELECT to_char(pk_sector,'X000'), UPPER(nb_sector), tx_descripcion,	pk_sector	FROM gan_sector";
		...
		<div class="page-content">
			...
			<div class="row">
				<div class="col-xs-12 col-sm-12 ">
					<button class="btn btn-success btn-sm pull-left " onclick="Agregar_Sector()">
								...
								<tbody>	
									<?php   
										if($tarea == "M"){
											...
											fun_dibujar_tabla($obj_miconexion,$li_numcampo,$li_indicecampo, 'LISTAR_SECTOR',0); // Dibuja la Tabla de Datos
...
<script type="text/javascript">
	...
	function Eliminar_Sector(identificador){
		if (confirm('Desea Eliminar este Registro?') == true){
			document.formulario.tarea.value = "E";
			document.formulario.pk_sector.value = identificador;
	...
	function Editar_Sector(identificador){
	...
	function Ver_Sector_Potrero(identificador){
		document.formulario.tarea.value = "X";
		document.formulario.pk_sector.value = identificador;
		document.formulario.action = "gan_sector_potrero_view.php";
		document.formulario.method = "POST";
		document.formulario.submit();
	}	
	
	
	function Agregar_Potrero(identificador){
		document.formulario.tarea.value = "X";
		document.formulario.pk_sector.value = identificador;
