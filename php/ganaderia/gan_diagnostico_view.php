...
-------------------------------------------------------------------------------------------*/
	if ($tarea == "E"){
		$ls_sql = "SELECT * FROM gan_palpacion WHERE fk_diagnostico= '$pk_diagnostico' ";
		
		$ls_resultado =  $obj_miconexion->fun_consult($ls_sql);
		if ($ls_resultado != 0){
			if($obj_miconexion->fun_numregistros($ls_resultado) == 0){ 
				$ls_sql = "DELETE FROM gan_diagnostico WHERE pk_diagnostico = '$pk_diagnostico' ";
				$ls_resultado =  $obj_miconexion->fun_consult($ls_sql);
					if($ls_resultado != 0){
...
-------------------------------------------------------------------------------------------*/	
	...
	$ls_sql = "SELECT UPPER(tx_descripcion), in_tipo_diagnostico, tx_comentario, pk_diagnostico 
							...
							<table id="dynamic-table" class="table table-striped table-bordered table-hover ">
								...
								<tbody>	
									<?php   
										...
										fun_dibujar_tabla($obj_miconexion,$li_numcampo,$li_indicecampo, 'LISTAR_DIAGNOSTICO',0); // Dibuja la Tabla de Datos
	...
	<script type="text/javascript">				
		...
		function Eliminar_Vacuna(identificador){
			if (confirm('Desea Eliminar este Registro?') == true){
				...
				document.formulario.pk_diagnostico.value = identificador;
				document.formulario.action = "gan_diagnostico_view.php";
				document.formulario.method = "POST";
				document.formulario.submit();
			}
		}
