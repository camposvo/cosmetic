...
------------------------------------------------------------------------------------------------------------------------*/
	if ($tarea == "U"){
		...
		$ls_sql = "UPDATE gan_ganado SET 
			in_activo= '$o_salida',
			fe_salida = '$o_fecha',
			tx_coment_salida = '$x_observacion'
...
-------------------------------------------------------------------------------------------*/
	$ls_sql = "SELECT id_numero, UPPER(nb_nombre_animal), in_sexo, to_char(fe_nacimiento, 'yyyy/mm/dd'), nb_potrero, nb_lote,
				f_grupo_etareo(fe_nacimiento,in_sexo), UPPER(gan_raza.nb_raza), 
				in_activo,	fe_salida,tx_coment_salida,
				pk_ganado
			FROM gan_ganado
		...
		<div class="row">	
			...
			<div class="col-xs-12 col-sm-8 ">						
				...
				<div class="form-group">
					...
					<div class="col-sm-7" >	
						<select name="o_salida" class=" chosen-select form-control" data-placeholder="Seleccione Vacuna...">
							<?php
								if ($o_salida == ""){
								...
								foreach($arr_salida as $k => $v) {
									$ls_cadenasel =($k == $o_salida)?'selected':'';
