...
-------------------------------------------------------------------------------------------*/
	if ($tarea == "E"){
		$ls_sql = "DELETE FROM gan_animal_vacuna
						WHERE pk_animal_vacuna = $pk_animal_vacuna;";		
...
------------------------------------------------------------------------------------------------------------------------*/
	if ($tarea == "U"){
		...
		$ls_sql = "INSERT INTO gan_animal_vacuna( fk_vacuna, fe_fecha_vacuna, fk_animal, tx_comentario ) 
			VALUES ( '$o_vacuna',  '$o_fecha', '$pk_animal','$x_observacion')";
...
-------------------------------------------------------------------------------------------*/			
	$ls_sql = "SELECT fe_fecha_vacuna, nb_vacuna, tx_dosis, tx_comentario, pk_animal_vacuna
				FROM gan_animal_vacuna
			LEFT JOIN gan_vacuna ON gan_vacuna.pk_vacuna = gan_animal_vacuna.fk_vacuna 
		...
		<div class="row">
			<div class="col-xs-12 col-md-12 col-lg-12">										
				<div class="table-header">
					Historico de Vacuna/Tratamiento
				</div>
				...
				<table id="dynamic-table" class="table table-striped table-bordered table-hover ">
					...
					<tbody>	
						<?php   										
							...
							fun_dibujar_tabla($obj_miconexion,$li_numcampo,$li_indicecampo, 'LISTAR_ANIMAL_SANIDAD'); // Dibuja la Tabla de Datos
			...
			<input type="hidden" name="pk_animal_vacuna" value="<?php echo $pk_animal_vacuna;?>">
...
<script language="javascript" type="text/javascript">
	...
	function Asignar(){
		if (confirm('Desea Asignar la vacuna?') == true){
	...
	function Eliminar_Vacuna(identificador){
		if (confirm('Desea Eliminar la vacuna?') == true){
			document.formulario.tarea.value = "E";
			document.formulario.pk_animal_vacuna.value = identificador;
			document.formulario.action = "gan_animal_sanidad.php";
			document.formulario.method = "POST";
			document.formulario.submit();
		}
