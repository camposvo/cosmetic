...
-------------------------------------------------------------------------------------------*/
	if ($tarea == "E"){
		$ls_sql = "DELETE FROM gan_secar_animal
						WHERE pk_secar_animal = $pk_secar;";		
...
------------------------------------------------------------------------------------------------------------------------*/
	if ($tarea == "U"){
		...
		$error_sql = false;			
		//$gender = $gender==1?'EN PRODUCCION':'FUERA DE PRODUCCION';
		
		
		$ls_sql = "UPDATE gan_secar_animal SET in_activo='N';		
		INSERT INTO gan_secar_animal( fe_fecha_produccion, in_produccion, in_activo, fk_animal ) 
			VALUES ( '$o_fecha','$gender','S', '$pk_animal')";
...
/*-------------------------------------------------------------------------------------------
	LEE EL REGISTRO ACTIVO QUE INDICA EN QUE ESTADO ESTA EL ANIMAL
-------------------------------------------------------------------------------------------*/
	$ls_sql = "SELECT to_char(fe_fecha_produccion, 'dd/mm/yyyy'), in_produccion 
				FROM gan_secar_animal
				WHERE fk_animal = $pk_animal AND in_activo = 'S'";		
	$ls_resultado =  $obj_miconexion->fun_consult($ls_sql);		
	...
	if ( $estado_prod == 1) {
		$Mensaje = '<div class="alert alert-block alert-success">
						<i class="ace-icon fa fa-check green"></i>
							El Animal esta en Produccion desde '.$fecha_prod.'
					</div>';

	}else if( $estado_prod == 2){
		 $Mensaje = '<div class="alert alert-block alert-warning">
						<i class="ace-icon fa fa-check yellow"></i>
							El Animal esta en Fuera de Produccion desde '.$fecha_prod.'
					</div>';
	}else{ 
		$Mensaje = '<div class="alert alert-block alert-info">
						<i class="ace-icon fa fa-check blue"></i>
							El Animal Nunca ha estado en Produccion 
					</div>';
	}
...
-------------------------------------------------------------------------------------------*/			
	$ls_sql = " SELECT fe_fecha_produccion, in_produccion, pk_secar_animal FROM gan_secar_animal	
	...
	<div class="container-fluid">		
		...
		<div class="col-xs-12 col-sm-8 left">
							...
							<div class="form-group">
								<label class="col-sm-3 control-label " for="id-date-picker-1" >Fecha</label>
								<div class="col-sm-3" >	
								...
								<div class="col-xs-12 col-sm-9 " >
									<div>
										<label class="line-height-1 ">
											...
											<span class="lbl left">En Produccion</span>
										</label>
									</div>
									<div>
										<label class="line-height-1 ">
											<input  name="gender" value="2" type="radio" class="ace" />
											<span class="lbl"> Fuera de Produccion</span>
										</label>
									</div>
								</div>
							</div>
			...
			<input type="hidden" name="pk_secar" value="<?php echo $pk_secar;?>">
...
<script language="javascript" type="text/javascript">
	...
	function Eliminar_Secar(identificador){
		if (confirm('Desea Eliminar el Registro?') == true){
			document.formulario.tarea.value = "E";
			document.formulario.pk_secar.value = identificador;
			document.formulario.action = "gan_produccion_secar.php";
			document.formulario.method = "POST";
			document.formulario.submit();
		}
